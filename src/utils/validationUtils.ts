...
export const personalInfoSchema = yup.object({
  ...
  zipCode: yup.string().required('Zip code is required').max(20, 'Zip code must be less than 20 characters'),
  country: yup.string().required('Country is required').max(50, 'Country must be less than 50 characters'),
...
export const experienceSchema = yup.object({
  ...
  company: yup.string().required('Company is required').max(100, 'Company name must be less than 100 characters'),
  ...
  location: yup.string().required('Location is required').max(100, 'Location must be less than 100 characters'),
  description: yup.string().required('Description is required').max(1000, 'Description must be less than 1000 characters'),
  achievements: yup.array().of(yup.string().max(200, 'Achievement must be less than 200 characters')).default([]),
  companyLogo: yup.string().optional(),
});

export const educationSchema = yup.object({
  id: yup.string().required(),
  institution: yup.string().required('Institution is required').max(100, 'Institution name must be less than 100 characters'),
  degree: yup.string().required('Degree is required').max(100, 'Degree must be less than 100 characters'),
  fieldOfStudy: yup.string().required('Field of study is required').max(100, 'Field of study must be less than 100 characters'),
  startDate: yup.string().required('Start date is required'),
  endDate: yup.string().required('End date is required'),
  gpa: yup.string().matches(/^[0-4](\.[0-9]{1,2})?$/, 'Invalid GPA format').optional(),
  location: yup.string().required('Location is required').max(100, 'Location must be less than 100 characters'),
...
export const skillSchema = yup.object({
  id: yup.string().required(),
  name: yup.string().required('Skill name is required').max(50, 'Skill name must be less than 50 characters'),
  level: yup.string().oneOf(['Beginner', 'Intermediate', 'Advanced', 'Expert']).required('Skill level is required'),
  category: yup.string().required('Category is required').max(50, 'Category must be less than 50 characters'),
});

export const projectSchema = yup.object({
  id: yup.string().required(),
  name: yup.string().required('Project name is required').max(100, 'Project name must be less than 100 characters'),
  description: yup.string().required('Description is required').max(1000, 'Description must be less than 1000 characters'),
...
export const certificationSchema = yup.object({
  id: yup.string().required(),
  name: yup.string().required('Certification name is required').max(100, 'Certification name must be less than 100 characters'),
  issuer: yup.string().required('Issuer is required').max(100, 'Issuer must be less than 100 characters'),
...
export const languageSchema = yup.object({
  id: yup.string().required(),
  name: yup.string().required('Language name is required').max(50, 'Language name must be less than 50 characters'),
  proficiency: yup.string().oneOf(['Basic', 'Conversational', 'Professional', 'Native']).required('Proficiency is required'),
});
...
export const validateResumeData = async (data: ResumeData): Promise<{ isValid: boolean; errors: Record<string, string> }> => {
  try {
