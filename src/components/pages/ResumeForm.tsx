...
const ResumeForm: React.FC<ResumeFormProps> = ({ fieldConfigs }) => {
  ...
  const sections = [
    {
      id: 'personalInfo',
      name: t('section.personalInfo'),
      icon: '👤',
      description: t('section.personalInfo.desc'),
    },
    {
      id: 'experience',
      name: t('section.experience'),
      icon: '💼',
      description: t('section.experience.desc'),
    },
    {
      id: 'education',
      name: t('section.education'),
      icon: '🎓',
      description: t('section.education.desc'),
    },
  ];
  ...
  const completedSections = sections.filter(section => {
    const sectionData =
      state.resumeData[section.id as keyof typeof state.resumeData];
        ...
        <div className='header-info'>
          ...
          <div className='progress-info'>
            ...
            <span className='progress-text'>
              {progressPercentage}% {t('progress.complete')} •{' '}
              {t('section.number')}{' '}
              {sections.findIndex(s => s.id === selectedSection) + 1}{' '}
              {t('section.of')} {sections.length}
            </span>
          </div>
        </div>
        ...
        <div className='form-sidebar'>
          <div className='section-navigation'>
            <h3 className='nav-title'>
              {t('resume.title')} {t('section.number')}
            </h3>
